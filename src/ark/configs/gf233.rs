//! Config for GF(2^233) with the NIST B-233 / K-233 polynomial — generic algorithms.
...
/// Config for GF(2^233).
///
/// Uses only generic algorithms (no polynomial-specific optimizations).
pub struct Gf233Config;

impl Gf233Config {
    /// The irreducible polynomial that defines this field.
    pub const POLY: IrreduciblePoly = polynomials::GF233_NIST;
}

impl BinaryFieldConfig<4> for Gf233Config {
    // 233 bits fit in 4 × 64-bit limbs, with 23 bits unused in limb 3.
    const DEGREE: usize = polynomials::GF233_NIST.degree;

    // f(z) = z^233 + z^74 + 1  (NIST B-233 / K-233)
    // α^233 = α^74 + 1:  bit 0 of limb 0 = 1,  bit 10 of limb 1 = 1<<10
    const ALPHA_POW_M: [u64; 4] = [
        polynomials::GF233_NIST.alpha_pow_m[0],
        polynomials::GF233_NIST.alpha_pow_m[1],
        polynomials::GF233_NIST.alpha_pow_m[2],
        polynomials::GF233_NIST.alpha_pow_m[3],
    ];
    ...
    // No overrides: mul_assign, square_in_place, and inverse all use
    // the generic defaults from BinaryFieldConfig.
}
...
mod tests {
    ...
    fn poly_constant_matches_config() {
        assert_eq!(Gf233Config::POLY.degree, 233);
        assert_eq!(Gf233Config::POLY.terms, &[74, 0]);
        assert_eq!(Gf233Config::POLY.alpha_pow_m, &[1u64, 1 << 10, 0, 0]);
    }
