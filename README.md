...
### ✅ Step 5 — Reduction, fast (`reduce_128_gf2p128`)
...
Cost reduction vs Algorithm 2.33: outer loop shrinks from m=128 to W=64 iterations; B is shifted 63 times instead of 127 times.
...
Square-and-multiply. Uses `square()` and `mul_comb()`.
...
|-----------------|---------------------|---------|-----------------------------------|
| `mul`           | 2.33                | ~104 ns | Baseline, inline reduction        |
| `mul_comb`      | 2.34                | ~51 ns | Comb method + fast reduction      |
| `square`        | 2.39                | ~2.8 ns | With fast reduction               |
| `invert`        | 2.48 (EEA)          | ~575 ns | Left-to-right, variable shifts    |
| `invert_binary` | 2.49 (binary GCD)   | ~286 ns | Right-to-left, fixed 1-bit shifts |
| `pow`           | square-and-multiply | ~13 µs  |                                   |
...
- ✅ `mul_comb(a, b)` — right-to-left comb + fast reduction (Algorithm 2.34)
- ✅ `square(a)` — bit-expansion via lookup table + fast reduction (Algorithm 2.39)

### Inversion
- ✅ `invert(a)` — EEA over GF(2)[x], left-to-right (Algorithm 2.48)
- ✅ `invert_binary(a)` — binary GCD, right-to-left (Algorithm 2.49)
